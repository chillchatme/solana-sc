use crate::{
    ...
    state::{ChillNftMetadata, Config, AUTHORITY_SHARE, CHILL_METADATA_SEED},
    utils::{self, assert, nft, pda},
};
...
pub fn process_mint_nft(
    ...
    let token_builder = nft::TokenBuilder {
        name: args.name,
        symbol: args.symbol,
        url: args.url,
    ...
    if authority.key != user.key {
        nft::sign_metadata(user, metadata, metadata_program)?;
        nft::set_primary_sell_happened(metadata, user, nft_token_account, metadata_program)?;
    }
