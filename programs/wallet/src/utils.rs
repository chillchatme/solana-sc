...
pub fn check_authority(authority: &Signer, proxy_wallet: &Account<ProxyWallet>) -> Result<()> {
    ...
    let proxy_wallet_pubkey;

    if authority_key == proxy_wallet.primary_wallet {
        proxy_wallet_pubkey = Pubkey::create_program_address(
            &[
                ProxyWallet::SEED,
        ...
        .map_err(|_| ErrorCode::WrongAuthority)?;
    ...
    require_keys_eq!(proxy_wallet.key(), proxy_wallet_pubkey);
